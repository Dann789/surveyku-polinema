...
class Dashboard {
    ...
    public function getDashboardData() {
        $query = "SELECT first_name, last_name, picture FROM p_alumni WHERE user_id = ?";
    ...
    public function getSurveyDone(){
        $query = "SELECT tj.survey_kategori_id, sk.survey_judul FROM t_jawaban_mahasiswa tj 
                    JOIN m_survey_kategori sk on tj.survey_kategori_id = sk.survey_kategori_id WHERE user_id = ?";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param("i", $this->user_id);
        $stmt->execute();
        $result = $stmt->get_result();
                    ...
                    <div class="group-survey d-flex align-items-baseline gap-5">
                        <div class="card" style="background-image: url(../../assets/images/rating.jpg); background-size: cover; background-repeat: no-repeat;">
                            <div class="card-body d-flex">
                              <h6 class="card-title fw-semibold mt-1">Kualitas Pendidikan</h6>
                            </div>
                        </div>
                        <div class="card" style="background-image: url(../../assets/images/fasilitas.jpg); background-size: cover; background-repeat: no-repeat;background-position-y: -20px;">
                            <div class="card-body d-flex">
                              <h6 class="card-title fw-semibold mt-1">Fasilitas Pengembangan Karir</h6>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="group-survey d-flex flex-column gap-4">
                            <div class="card" style="background-image: url(../../assets/images/layanan.jpg); background-size: cover; background-repeat: no-repeat;">
                                <div class="card-body d-flex flex-column justify-content-between">
                                  <h6 class="card-subtitle fw-semibold">Proses Administrasi Alumni</h6>
                                </div>
                            </div>
                            <div class="card" style="background-image: url(../../assets/images/img-mhs.png); background-size: cover; background-repeat: no-repeat;background-position-y: -20px;">
                                <div class="card-body d-flex flex-column justify-content-between" >
                                  <h6 class="card-subtitle fw-semibold">Jaringan Alumni</h6>
                                </div>
                            </div>
                    </div>
                </aside>
